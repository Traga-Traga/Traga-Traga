      ...
      //Titulo c:
        $("head").append('<h2 class="FirstTitle" style="text-align:center; ">ƇƖαη 邪神 Agαяισ</h2>');  
         //imagen mega grande
           ...
           $("div#mainPanel.agario-panel").append('<div class="form-group clearfix"></div>');
            ...
            $("div#mainPanel.agario-panel").append('<h2 class="FirstTitle" style="text-align:center; ">ƇƖαη 邪神 Agαяισ</h2>'); 
...
$("#hide_url").click(function() {
    if (this.value == 'hide') {
        ...
        $("#skin_url").css('padding-left', '12px')
                ...
                $("#profile-pic").prepend('<div id="profileShortContainer" style="margin-bottom:15px;height:27px"><button class="profileShort" value="💕">💕</button><button class="profileShort" value="T">T</button><button class="profileShort" value="R">R</button><button class="profileShort" value="A">A</button><button class="profileShort" value="G">G</button><button class="profileShort" value="A">A</button><button class="profileShort" value="❥">❥</button></div>');
                    ...
                    var profileFilter = JSON.parse(playerProfiles)[JSON.parse(currentProfile) - 1];
                ...
                function checkIP_Token(){
                    ...
                    if(tkip.indexOf(".")==2 || tkip.indexOf(".")==3){
                ...
                $("#addMenuKontainer").after('<div id="additionalPanelK" class=" agario-side-panel" style="margin-top:0px;display:none;padding-top:0px !important;font-size:13px !important;font-family:Ubuntu"></div>');
                ...
                var ndSett = $(".col-xs-6.secondSettings");
                panelPlusK.append("<input id='opt_privateMode' type='checkbox'> Private Mode<br>");
                var privateElems = $("#team_name,#region,#connect_to_ip,.partytoken,#ip_info,#region_info,#gamemode_info");
                
                if(JSON.parse(localStorage.getItem("opt_privateMode"))===true){
                    $("#opt_privateMode").prop("checked",true);
                        privateElems.addClass("privateModeK");
                } else if(JSON.parse(localStorage.getItem("opt_privateMode"))===false){
                    $("#opt_privateMode").prop("checked",false);
                } else {
                    localStorage.setItem("opt_privateMode",false);
                }
                $("#opt_privateMode").change(function(){
                    if($(this).is(":checked")){
                        privateElems.addClass("privateModeK");
                        localStorage.setItem("opt_privateMode",true);
                    } else {
                        privateElems.removeClass("privateModeK");
                        localStorage.setItem("opt_privateMode",false);
                    }
                });
                
                panelPlusK.append("<input id='opt_infoBoxK' type='checkbox'> Hide Name Editor<br>");
